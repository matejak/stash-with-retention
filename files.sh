...
# ARG_POSITIONAL_SINGLE([regexp],[What files to look for in the shortterm directory],['*'])
# ARG_OPTIONAL_SINGLE([shortterm-time],[s],[Shortterm storage cap],[31])
# ARG_OPTIONAL_SINGLE([longterm-time],[l],[Longterm storage cap],[24])
# ARG_OPTIONAL_SINGLE([longerm-regexp],[r],[Longterm content match],[-01($|[^-]])
...
_arg_longerm_regexp="-01($|[^-]"

print_help ()
{
	printf "%s\n" "The general script's help msg"
	printf 'Usage: %s [-s|--shortterm-time <arg>] [-l|--longterm-time <arg>] [-r|--longerm-regexp <arg>] [-h|--help] [<regexp>]\n' "$0"
	printf "\t%s\n" "<regexp>: What files to look for in the shortterm directory (default: '"'*'"')"
	printf "\t%s\n" "-s,--shortterm-time: Shortterm storage cap (default: '"31"')"
	printf "\t%s\n" "-l,--longterm-time: Longterm storage cap (default: '"24"')"
	printf "\t%s\n" "-r,--longerm-regexp: Longterm content match (default: '"-01($|[^-]"')"
	printf "\t%s\n" "-h,--help: Prints help"
}
...
parse_commandline ()
{
	...
	do
		...
		case "$_key" in
			...
			-r|--longerm-regexp)
				test $# -lt 2 && die "Missing value for the optional argument '$_key'." 1
				_arg_longerm_regexp="$2"
				shift
				;;
			--longerm-regexp=*)
				_arg_longerm_regexp="${_key##--longerm-regexp=}"
				;;
			-r*)
				_arg_longerm_regexp="${_key##-r}"
				;;
			-h|--help)
				print_help
				exit 0
...
source "$script_dir/src.sh"

handle_directory "$_arg_shortterm_regexp" "shortterm" "$_arg_shortterm_time" "$_arg_longterm_regexp" "longterm" "$_arg_longterm_time"
